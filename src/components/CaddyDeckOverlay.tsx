...
// One tap flips the caddy pack open with a continuous rumble that lasts until every caddy card is
// on screen. The reveal timing below matches the grid's FadeInDown (delay i*45, dur 320).
const FLIP_MS = 520;
...
export function CaddyDeckOverlay({ onOpen, onClose }: Props) {
  ...
  const open = () => {
    ...
    // One tap flips the pack open with a continuous rumble that runs until every caddy is revealed.
    jigglePack();
    playGolfHit();
